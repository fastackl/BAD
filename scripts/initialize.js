...
const fastUtils = require(path.join(__dirname, '../scripts/fastUtils/utils.js'));
...
const scriptsConfigNetwork = hre.network.name === 'hardhat' ? 'localhost' : hre.network.name;
const provider = network === 'localhost' ? new ethers.providers.JsonRpcProvider() : new ethers.providers.StaticJsonRpcProvider(hre.config.networks[network].url);
...
async function readDeploymentData(contractName){
	try {
		return readJSONFromFile(path.join(__dirname, '../deployments', `${contractName}.json`));
	} catch (e) {
...
async function processFunctionArgs(args) {
    return await Promise.all(args.map(async (arg) => {
        if (Array.isArray(arg)) {
            ...
            return processFunctionArgs(arg);
        } else if (typeof arg === 'string') {
            if (arg.endsWith('.address')) {
                // If the argument is a string ending with '.address', fetch the actual address
        ...
        // If the argument is neither an array nor a string ending with '.address', return it as is
        return arg;
    }));
}
...
function createConsoleTable() {
	...
	stream.write([colors.cyan('Contract'), colors.cyan('Address'), colors.cyan('Function'), colors.cyan('Arguments'), colors.cyan('Response')]);
	return stream;
}
...
async function initializeContract(init, signer) {
	...
	try {
	    ...
	    const contract = new ethers.Contract(contractDeploymentData.address, contractDeploymentData.abi, signer);
		const tx = await contract[init.function](...args, { gasPrice: ethers.utils.parseUnits('10', 'gwei') });
	    let receipt = await tx.wait();

	    // Map each argument to its corresponding address
	    let argsWithAddresses = await Promise.all(init.args.map(async (arg) => {
	    ...
	    return { contract: init.contract, address: contractDeploymentData.address, function: init.function, args: argsWithAddresses.join('\n\n') + colors.grey('\nSource: ') + colors.grey(init.argsSource), response: {type: 'success', message: receipt} };
	} catch (e) {
		...
		return { contract: init.contract, address: contractDeploymentData ? contractDeploymentData.address : 'N/A', function: init.function, args: argsWithAddresses.join('\n\n') + colors.grey('\nSource: ') + colors.grey(init.argsSource), response: {type: 'error', message: e.message} };
	}
}
...
async function main(configArray) {
    ...
    const results = [];
    for (const init of INITIALIZE_CONFIG) {
        try {
            const result = await initializeContract(init, signer);
            results.push(result);
        } catch (e) {
            results.push({ contract: init.contract, address: 'N/A', function: init.function, args: 'N/A', response: {type: 'error', message: e.message} });
        }
    }

    results.forEach(result => {
        let responseColor = result.response.type === 'error' ? colors.red : colors.green;
        table.write([result.contract, result.address, result.function, result.args, responseColor(JSON.stringify(result.response.message))]);
    });
}
